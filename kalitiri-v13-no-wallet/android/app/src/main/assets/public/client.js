...
function renderSeats(){
  ...
  state.players.forEach((p,i)=>{
    const relative=(i-state.viewerIndex+n)%n;
    const angle=(90+(360*relative/n))*Math.PI/180;
    let x=50+43.5*Math.cos(angle),y=50+38.5*Math.sin(angle);
    if(i===state.viewerIndex){x=50;y=87;}
    const seat=document.createElement("div");
    seat.dataset.playerIndex=String(i);
    seat.className=`seat ${i===state.viewerIndex?"viewer":"opponent"} ${state.turnIndex===i&&state.phase==="playing"?"active":""} ${p.voiceJoined?"voice-connected":""} ${p.voiceMuted?"voice-muted":""} ${!p.connected&&!p.bot?"reconnecting":""}`;
    ...
    const isTurn=state.turnIndex===i&&state.phase==="playing";
    const reconnectBadge=!p.connected&&!p.bot?`<span class="reconnect-badge">↻ BOT ASSIST</span>`:"";
...
function renderTrick(){
  ...
  state.trick.forEach(play=>{
    const relative=(play.playerIndex-state.viewerIndex+n)%n;
  ...
  if(state.lastTrick && !state.trick.length && state.phase==="playing"){
    const winner=state.players[state.lastTrick.winnerIndex];
    const flash=document.createElement("div");flash.className="trick-winner-flash";
...
function renderHand(){
  ...
  const handTotal=state.hand.length;
  state.hand.forEach((card,cardIndex)=>{
    ...
    if(can){
      ...
      const play=()=>animateCardPlay(el,()=>socket.emit("playCard",{cardId:card.id},res=>{if(res&&!res.ok){el.classList.remove("card-launching");toast(res.error);}}));
...
$("createForm").addEventListener("submit",e=>{
  ...
  socket.emit("createRoom",{name:p.name,avatar:p.avatar,playerCount:Number($("playerCount").value)},res=>{if(!res?.ok)return toast(res?.error||"Could not create room.");saveSession(res.code,res.reconnectToken);});
});
$("joinForm").addEventListener("submit",e=>{
  e.preventDefault();const p=currentProfile();saveProfile();
  socket.emit("joinRoom",{name:p.name,avatar:p.avatar,code:$("joinCode").value.toUpperCase()},res=>{if(!res?.ok)return toast(res?.error||"Could not join room.");saveSession(res.code,res.reconnectToken);});
});
...
socket.on("state",next=>{
  ...
  if(prev&&prev.phase==="playing"&&next.phase==="roundEnd")beep("win");
...
socket.on("roomClosed",()=>{clearSession();toast(language==="gu"?"રૂમ બંધ થઈ ગયો.":"Room closed or expired.");setTimeout(()=>location.reload(),900);});
...
socket.on("connect",()=>{setNetStatus("online","● Online");tryResumeSession(false);});
...
setInterval(()=>{
  if(!state||!state.actionDeadline)return;
